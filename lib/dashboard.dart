...
class _DashportState extends State<Dashport> {
  ...
  void polygonBuilder(LineNotifier lineNotifier, LatLng startingCoordinate_, LatLng endCoordinate_, int connID){
    borderDouble = getBorderBetween(startingCoordinate_.latitude, startingCoordinate_.longitude,
                                    endCoordinate_.latitude, endCoordinate_.longitude, 10);
  ...
  void cityPoly(LineNotifier lineNotifier) {
    //if (lineNotifier.isEnergyActive != oldStatus) {
      border.clear();
      _polygon.clear();
      for (var i = 0; i < 4; i++) {
        if (i != 1) {
    ...
    //oldStatus = lineNotifier.isEnergyActive;
  //}
